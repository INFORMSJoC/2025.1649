...
namespace Clipper2Lib
{
  ...
  static const Rect invalid_rect = Rect(false);
  ...
  inline double AreaTriangle(const Point &pt1, const Point &pt2, const Point &pt3)
  {
  ...
  inline bool PtsReallyClose(const Point &pt1, const Point &pt2)
  {
  ...
  static PointInPolygonResult PointInOpPolygon(const Point &pt, OutPt *op)
  {
  ...
  inline Path GetCleanPath(OutPt *op)
  {
    Path result;
    OutPt *op2 = op;
    while (op2->next != op &&
  ...
  inline bool Path1InsidePath2(OutPt *op1, OutPt *op2)
  {
    ...
    Point mp = GetBounds(GetCleanPath(op1)).MidPoint();
    Path path2 = GetCleanPath(op2);
  ...
  void AddPaths_(const Paths &paths, PathType polytype, bool is_open, std::vector<Vertex *> &vertexLists, LocalMinimaList &locMinList)
  {
    const auto total_vertex_count =
        std::accumulate(paths.begin(), paths.end(), 0,
                        [](const auto &a, const Path &path)
  ...
  //------------------------------------------------------------------------------
  // ReuseableDataContainer methods ...
  //------------------------------------------------------------------------------

  void ReuseableDataContainer::AddLocMin(Vertex &vert, PathType polytype, bool is_open)
  {
    // make sure the vertex is added only once ...
    if ((VertexFlags::LocalMin & vert.flags) != VertexFlags::None)
      return;
  ...
  void ReuseableDataContainer::AddPaths(const Paths &paths, PathType polytype, bool is_open)
  {
    AddPaths_(paths, polytype, is_open, vertex_lists_, minima_list_);
  }

  ReuseableDataContainer::~ReuseableDataContainer()
  {
    Clear();
  }

  void ReuseableDataContainer::Clear()
  {
    minima_list_.clear();
  ...
  void ClipperBase::AddPath(const Path &path, PathType polytype, bool is_open)
  {
    Paths tmp;
    tmp.push_back(path);
  ...
  void ClipperBase::AddPaths(const Paths &paths, PathType polytype, bool is_open)
  {
    if (is_open)
  ...
  void ClipperBase::AddReuseableData(const ReuseableDataContainer &reuseable_data)
  {
  ...
  OutPt *ClipperBase::AddLocalMinPoly(Active &e1, Active &e2, const Point &pt, bool is_new)
  {
  ...
  OutPt *ClipperBase::AddLocalMaxPoly(Active &e1, Active &e2, const Point &pt)
  {
    if (IsJoined(e1))
      Split(e1, pt);
    if (IsJoined(e2))
  ...
  OutPt *ClipperBase::AddOutPt(const Active &e, const Point &pt)
  {
  ...
  OutPt *ClipperBase::StartOpenPath(Active &e, const Point &pt)
  {
    OutRec *outrec = NewOutRec();
    outrec->is_open = true;
  ...
  OutPt *ClipperBase::IntersectEdges(Active &e1, Active &e2, const Point &pt)
  {
  ...
  void ClipperBase::AddNewIntersectNode(Active &e1, Active &e2, int64_t top_y)
  {
    ...
    if (!GetIntersectPoint(e1.bot, e1.top, e2.bot, e2.top, ip))
      ip = Point(e1.curr_x, top_y); // parallel edges
  ...
  {
    Point nextPt = NextVertex(horzEdge)->pt;
   ...
   *******************************************************************************/
  {
    ...
    {
#ifdef USINGZ
      OutPt *op = AddOutPt(horz, Point(horz.curr_x, y, horz.bot.z));
#else
      OutPt *op = AddOutPt(horz, Point(horz.curr_x, y));
#endif
      AddTrialHorzJoin(op);
    }
  ...
  void ClipperBase::Split(Active &e, const Point &pt)
  {
  ...
  void ClipperBase::CheckJoinLeft(Active &e,
                                  const Point &pt, bool check_curr_x)
  {
    Active *prev = e.prev_in_ael;
  ...
  bool BuildPath(OutPt *op, bool reverse, bool isOpen, Path &path)
  {
  ...
  void ClipperBase::RecursiveCheckOwners(OutRec *outrec, PolyPaths *polypath)
  {
  ...
  bool BuildPath(OutPt *op, bool reverse, bool isOpen, Path &path, double inv_scale)
  {
    if (!op || op->next == op || (!isOpen && op->next == op->prev))
      return false;
    ...
    path.push_back(Point(lastPt.x * inv_scale, lastPt.y * inv_scale));
  ...
  void Clipper::BuildPaths(Paths &solutionClosed, Paths *solutionOpen)
  {
    ...
    for (std::size_t i = 0; i < outrec_list_.size(); ++i)
    {
      ...
      {
        if (BuildPath(outrec->pts, ReverseSolution, true, path, invScale_))
      ...
      else
      {
        ...
        if (BuildPath(outrec->pts, ReverseSolution, false, path, invScale_))
          solutionClosed.emplace_back(std::move(path));
      }
    }
  }

  void Clipper::BuildTree(PolyPath &polytree, Paths &open_paths)
  {
    polytree.Clear();
    open_paths.resize(0);
    if (has_open_paths_)
    ...
    // outrec_list_.size() is not static here because
    // BuildPath below can indirectly add additional OutRec //#607
