...
namespace Clipper2Lib
{
	...
	void GetBoundsAndLowestPolyIdx(const Paths &paths, Rect &r, int &idx)
	{
		idx = -1;
		r = MaxInvalidRect;
		int64_t lpx = 0;
		for (int i = 0; i < static_cast<int>(paths.size()); ++i)
			for (const Point &p : paths[i])
			{
				if (p.y >= r.bottom)
				{
	...
	bool IsSafeOffset(const Rect &r, double abs_delta)
	{
	...
	Point GetUnitNormal(const Point &pt1, const Point &pt2)
	{
	...
	inline Point NormalizeVector(const Point &vec)
	{
		double h = Hypot(vec.x, vec.y);
		if (AlmostZero(h))
			return Point(0, 0);
		double inverseHypot = 1 / h;
		return Point(vec.x * inverseHypot, vec.y * inverseHypot);
	}

	inline Point GetAvgUnitVector(const Point &vec1, const Point &vec2)
	{
		return NormalizeVector(Point(vec1.x + vec2.x, vec1.y + vec2.y));
	}
	...
	inline Point GetPerpendic(const Point &pt, const Point &norm, double delta)
	{
		return Point(pt.x + norm.x * delta, pt.y + norm.y * delta);
	}

	inline void NegatePath(Path &path)
	{
		for (Point &pt : path)
		{
			pt.x = -pt.x;
	...
	void ClipperOffset::AddPath(const Path &path, JoinType jt_, EndType et_)
	{
		Paths paths;
	...
	void ClipperOffset::AddPaths(const Paths &paths, JoinType jt_, EndType et_)
	{
		if (paths.size() == 0)
			return;
	...
	void ClipperOffset::BuildNormals(const Path &path)
	{
		norms.clear();
		norms.reserve(path.size());
		if (path.size() == 0)
			return;
		Path::const_iterator path_iter, path_last_iter = --path.cend();
	...
	inline Point TranslatePoint(const Point &pt, double dx, double dy)
	{
		return Point(pt.x + dx, pt.y + dy);
	}

	inline Point ReflectPoint(const Point &pt, const Point &pivot)
	{
		return Point(pivot.x + (pivot.x - pt.x), pivot.y + (pivot.y - pt.y));
	}

	Point IntersectPoint(const Point &pt1a, const Point &pt1b, const Point &pt2a, const Point &pt2b)
	{
		if (pt1a.x == pt1b.x) // vertical
		{
			...
			return Point(pt1a.x, m2 * pt1a.x + b2);
		}
		else if (pt2a.x == pt2b.x) // vertical
		{
			double m1 = (pt1b.y - pt1a.y) / (pt1b.x - pt1a.x);
			double b1 = pt1a.y - m1 * pt1a.x;
			return Point(pt2a.x, m1 * pt2a.x + b1);
		}
		else
		{
			double m1 = (pt1b.y - pt1a.y) / (pt1b.x - pt1a.x);
	...
	void ClipperOffset::DoBevel(Group &group, const Path &path, size_t j, size_t k)
	{
		Point pt1, pt2;
		if (j == k)
		{
			double abs_delta = std::abs(group_delta_);
			pt1 = Point(path[j].x - abs_delta * norms[j].x, path[j].y - abs_delta * norms[j].y);
			pt2 = Point(path[j].x + abs_delta * norms[j].x, path[j].y + abs_delta * norms[j].y);
		}
		else
		{
			pt1 = Point(path[j].x + group_delta_ * norms[k].x, path[j].y + group_delta_ * norms[k].y);
			pt2 = Point(path[j].x + group_delta_ * norms[j].x, path[j].y + group_delta_ * norms[j].y);
		}
		group.path.push_back(Point(pt1));
		group.path.push_back(Point(pt2));
	}

	void ClipperOffset::DoSquare(Group &group, const Path &path, size_t j, size_t k)
	{
		Point vec;
		if (j == k)
			vec = Point(norms[j].y, -norms[j].x);
		else
			vec = GetAvgUnitVector(
				Point(-norms[k].y, norms[k].x),
				Point(norms[j].y, -norms[j].x));

		double abs_delta = std::abs(group_delta_);

		// now offset the original vertex delta units along unit vector
		Point ptQ = Point(path[j]);
		ptQ = TranslatePoint(ptQ, abs_delta * vec.x, abs_delta * vec.y);
		// get perpendicular vertices
		Point pt1 = TranslatePoint(ptQ, group_delta_ * vec.y, group_delta_ * -vec.x);
		Point pt2 = TranslatePoint(ptQ, group_delta_ * -vec.y, group_delta_ * vec.x);
		// get 2 vertices along one edge offset
		Point pt3 = GetPerpendic(path[k], norms[k], group_delta_);
		if (j == k)
		{
			Point pt4 = Point(pt3.x + vec.x * group_delta_, pt3.y + vec.y * group_delta_);
			Point pt = IntersectPoint(pt1, pt2, pt3, pt4);
			// get the second intersect point through reflecion
			group.path.push_back(Point(ReflectPoint(pt, ptQ)));
			group.path.push_back(Point(pt));
		}
		else
		{
			Point pt4 = GetPerpendic(path[j], norms[k], group_delta_);
			Point pt = IntersectPoint(pt1, pt2, pt3, pt4);
			group.path.push_back(Point(pt));
			// get the second intersect point through reflecion
			group.path.push_back(Point(ReflectPoint(pt, ptQ)));
	...
	void ClipperOffset::DoMiter(Group &group, const Path &path, size_t j, size_t k, double cos_a)
	{
		double q = group_delta_ / (cos_a + 1);
		group.path.push_back(Point(
			path[j].x + (norms[k].x + norms[j].x) * q,
			path[j].y + (norms[k].y + norms[j].y) * q));
	}

	void ClipperOffset::DoRound(Group &group, const Path &path, size_t j, size_t k, double angle)
	{
		if (deltaCallback64_)
		{
		...
		Point pt = path[j];
		Point offsetVec = Point(norms[k].x * group_delta_, norms[k].y * group_delta_);

		if (j == k)
			offsetVec = -1 * offsetVec;

		int steps = static_cast<int>(std::ceil(steps_per_rad_ * std::abs(angle))); // #448, #456
		for (int i = 1; i < steps; ++i)											   // ie 1 less than steps
		{
			offsetVec = Point(offsetVec.x * step_cos_ - step_sin_ * offsetVec.y,
							  offsetVec.x * step_sin_ + offsetVec.y * step_cos_);
		}
		group.path.push_back(GetPerpendic(path[j], norms[j], group_delta_));
	}

	void ClipperOffset::OffsetPoint(Group &group, Path &path, size_t j, size_t k)
	{
	...
	void ClipperOffset::OffsetPolygon(Group &group, Path &path)
	{
		...
		{
			Rect rec = GetBounds(path);
			double offsetMinDim = std::fabs(group_delta_) * 2;
			if (offsetMinDim > rec.Width() || offsetMinDim > rec.Height())
				return;
		}

		for (Path::size_type j = 0, k = path.size() - 1; j < path.size(); k = j, ++j)
			OffsetPoint(group, path, j, k);
		group.paths_out.push_back(group.path);
	}

	void ClipperOffset::OffsetOpenJoined(Group &group, Path &path)
	{
	...
	void ClipperOffset::OffsetOpenPath(Group &group, Path &path)
	{
		...
		for (Path::size_type j = 1, k = 0; j < highI; k = j, ++j)
			OffsetPoint(group, path, j, k);

		// reverse normals
		for (size_t i = highI; i > 0; --i)
			norms[i] = Point(-norms[i - 1].x, -norms[i - 1].y);
		norms[0] = norms[highI];
		...
		for (path_iter = group.paths_in.begin(); path_iter != group.paths_in.end(); ++path_iter)
		{
			...
			if (cnt == 1) // single point - only valid with open paths
			{
				...
				else
				{
					...
					r = Rect(path[0].x - d, path[0].y - d, path[0].x + d, path[0].y + d);
					group.path = r.AsPath();
				}
				group.paths_out.push_back(group.path);
			}
	...
	void ClipperOffset::Execute(double delta, Paths &paths)
	{
		paths.clear();
	...
	void ClipperOffset::Execute(double delta, PolyTree &polytree)
	{
		polytree.Clear();
	...
	void ClipperOffset::Execute(DeltaCallback delta_cb, Paths &paths)
	{
