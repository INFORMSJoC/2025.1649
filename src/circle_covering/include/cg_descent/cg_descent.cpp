...
namespace CG_DESCENT
{
        ...
        )
    {
        ...
        for (iter = 1; iter <= maxit; iter++)
        {
            ...
            if (!UseMemory)
            {
                if (!LBFGS)
                {
                    if ((IterRestart >= nrestart) || ((IterQuad == qrestart) && (IterQuad != IterRestart)))
                        Restart = TRUE;
                }
            }
            else
            ...
            else /* compute the search direction in the full space */
            {
                ...
                else /* FirstFull = TRUE, precondition after leaving subspace */
                {
                    ...
                    if (Parm->AdaptiveBeta)
                        ...
                        t = Parm->theta;
                    t1 = MAX(ykyk - yty, ZERO_); /* Theoretically t1 = ykyk-yty */
                    if (ykyk > ZERO_)
                    {
        ...
        for (iter = 0; iter < Parm->nline; iter++)
        {
            ...
            if ((toggle == 0) || ((toggle == 2) && ((b - a) <= width)))
            {
                ...
                if (Com->UseCubic && qb)
                {
                    ...
                    if (alpha < ZERO_) /* use secant method */
                    {
                        s1 = "secant   ";
                        if (-da < db)
    ...
    )
    {
        ...
        t = v * v - da * db;
        if (t < ZERO_) /* complex roots, use secant method */
        {
            if (fabs(da) < fabs(db))
                c = a - (a - b) * (da / (da - db));
            else if (da != db)
