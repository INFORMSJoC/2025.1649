...
void save_region(std::string path)
{
    // save container data
    std::stringstream ss;
    ss << path + "/container.csv";
    std::string filename = ss.str();
    std::ofstream outfile(filename);
    for (size_t i = 0; i < border.size(); i++)
        outfile << border[i].x << "," << border[i].y << "\n";
    outfile << border[0].x << "," << border[0].y << "\n";
    outfile.close();
    ...
    {
        std::stringstream ss;
        ss << path + "/holes/hole_" << i << ".csv";
        std::string filename = ss.str();
        std::ofstream outfile(filename);
        for (size_t j = 0; j < holes[i].size(); j++)
            outfile << holes[i][j].x << "," << holes[i][j].y << "\n";
        outfile << holes[i][0].x << "," << holes[i][0].y << "\n";
        outfile.close();
    }
}
...
void save_result(std::vector<Point> points, double radius, std::string path)
{
    ...
    {
        voronoi[i] = convexhull(voronoi[i]);
        std::stringstream ss;
        ss << path + "/voronoi/data_" << i << ".csv";
        std::string filename = ss.str();
        std::ofstream outfile(filename);
        outfile << points[i].x << "," << points[i].y << "\n";
        // std::cout << i << std::endl;
        for (size_t j = 0; j < voronoi[i].size(); ++j)
            outfile << voronoi[i][j].x << "," << voronoi[i][j].y << "\n";
        outfile << voronoi[i][0].x << "," << voronoi[i][0].y << "\n";
        outfile.close();
    }

    {
        ...
        std::ofstream outfile(filename);
        for (size_t i = 0; i < points.size(); i++)
            outfile << points[i].x << "," << points[i].y << "," << radius << "\n";
        outfile.close();
    }
}
